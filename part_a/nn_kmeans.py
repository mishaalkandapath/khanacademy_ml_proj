...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, output):
        ...
        # #         if i != j:
        # #             loss += torch.clamp(self.margin - (outputs[i] - outputs[j]).abs(), min=0)
        # for cluster in q_clusters_15subjects:
        ...
        #     answers = outputs[:,questions]
        #     #get the similarity between the answers
        #     similarity = torch.mm(answers, torch.t(answers))
        #     #get the diagonal of the similarity matrix
        #     diag = torch.diag(similarity)
        #     #get the similarity between the question and the rest of the questions in the cluster
        #     similarity = similarity - diag
        #     #get the max similarity between the question and the rest of the questions in the cluster
        #     max_similarity = torch.max(similarity, dim=1)[0]
        #     #get the min similarity between the question and the rest of the questions in the cluster
        #     min_similarity = torch.min(similarity, dim=1)[0]
        #     #get the difference between the max and min similarity
        #     diff = max_similarity - min_similarity
        #     #get the mean of the difference
        #     mean = torch.mean(diff)
        #     #get the loss
        #     loss += torch.mean(torch.max(torch.zeros_like(mean), self.margin - mean))
        # return loss
        # for each cluster: avg/var
    # r = 1 - (variance / (n * standard deviation^2))
        global q_clusters_15subjects        # 35 clusters
        q_clusters_15subjects = torch.LongTensor(q_clusters_15subjects).unsqueeze(0)
        # global q_clusters_30subjects  # 53 clusters
        src = torch.from_numpy(np.asarray([1, 2, 3, 4, 5]))
        torch.zeros(3, 5, dtype=src.dtype).scatter_(0, torch.from_numpy(np.asarray([0, 1, 2])), src)
        print(torch.ones(1, 1774).shape, output.shape)
        ones_matrix = torch.zeros(36, 1774, dtype=output.dtype).scatter_(0, q_clusters_15subjects, output)
        matrix = torch.zeros(36, 1774, dtype=output.dtype).scatter_(0, q_clusters_15subjects, output)  # 1667 x 35 matrix
        avg = torch.sum(matrix, dim=1)/(torch.count_nonzero(matrix, dim=1))     # vector of avgs for each row
        avg = np.column_stack([avg.detach().numpy()]* 1774)
        ones_matrix = torch.zeros(36, 1774, dtype=output.dtype).scatter_(0, q_clusters_15subjects, output)
        average_matrix = ones_matrix*torch.from_numpy(avg)
        # avgs = torch.zeros(36, 1774, dtype=output.dtype).scatter_(1, q_clusters_15subjects, average)  # 1667 x 35 matrix
        var = torch.sum(torch.square(matrix - average_matrix), dim=1)/(torch.count_nonzero(matrix, dim=1))
        loss = -torch.sum(var/(torch.sqrt(var)))
